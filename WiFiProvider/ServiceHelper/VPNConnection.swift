...
//  Q4U_VPNAPP
//
//  Created by Deepti Chawla on 25/03/21.
...
public class Connection: NSObject, ObservableObject {
    ...
    public init(profile: Profile) {
        ...
        loadProviderManager {
            ...
            NotificationCenter.default.addObserver(forName: NSNotification.Name.NEVPNStatusDidChange,
                ...
                self.connectionStatus = self.providerManager.connection.status
                Log.append(Util.localize("connection-status-changed\(self.providerManager.connection.status.description)", self.providerManager.connection.status.description), .info, .mainApp)
    ...
    public func setCustomConfigFile(url: String) {
        print("setCustomConfigFile url \(url)")
        guard let url = URL(string: url) else {
            print("setCustomConfigFile url nill")
            return
        }
  
        URLSession.shared.dataTask(with: url) { data, response, error in
            guard let data = data, error == nil else {
                print("setCustomConfigFile error")
                return }
//            let tmpURL = FileManager.default.temporaryDirectory
//                .appendingPathComponent(response?.suggestedFilename ?? "tempvpn.ovpn")
//            do {
...
//            }
            ...
            print("setCustomConfigFile\(data)")
                //let data = try Data(contentsOf: url!)
                self.setConfigFile(configFile: data)
    ...
    public func setConfigFile(configFile: Data) {
        ...
        print("SetConfig \(profile.serverAddress)\(profile.dnsList)")
    }
